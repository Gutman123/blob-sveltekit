<script lang="ts">
  ...
  let memberLevel = $derived(() => {
    ...
    if (data.subscription) {
      return { label: "白金会员", color: "#E5E7EB", icon: "💎", days };
    }
    if (days >= 365)
      return { label: "黄金会员", color: "#F59E0B", icon: "👑", days };
    if (days >= 90)
      return { label: "白银会员", color: "#94A3B8", icon: "🥈", days };
  ...
  let progressTarget = $derived(
    memberLevel().days >= 365 ? 365 : memberLevel().days >= 90 ? 365 : 90,
  );
  let progressPct = $derived(
    Math.min(100, Math.round((memberLevel().days / progressTarget) * 100)),
  );
</script>
    ...
    <div class="header-card">
      ...
      <div class="header-content">
        <div class="avatar-wrap">
          ...
          <div
            class="member-badge"
            style="background: {memberLevel()
              .color}22; border-color: {memberLevel().color}55"
          >
      ...
      <div class="tabs">
        ...
        <div
          class="tab-indicator"
          style="left: calc({activeTab === 'membership'
            ? 0
            : activeTab === 'profile'
              ? 1
              : 2} * 33.333%)"
    ...
    <div class="panel">
      ...
      {#if activeTab === "membership"}
        <div class="panel-section fade-in">
          ...
          <div class="tier-grid">
            {#each [{ label: "普通会员", icon: "✨", color: "#6EE7B7", bg: "#6EE7B722", req: "注册即可获得", perks: ["基础文件上传", "50MB 单文件限制", "标准访问速度"] }, { label: "白银会员", icon: "🥈", color: "#94A3B8", bg: "#94A3B822", req: "注册满 90 天", perks: ["优先文件处理", "200MB 单文件限制", "加速访问节点", "专属客服支持"] }, { label: "黄金会员", icon: "👑", color: "#F59E0B", bg: "#F59E0B22", req: "注册满 365 天", perks: ["最高优先级", "无限制文件大小", "专属 CDN 加速", "高级数据分析"] }, { label: "白金会员", icon: "💎", color: "#E5E7EB", bg: "#E5E7EB22", req: "付费订阅 ($10/月起)", perks: ["所有黄金特权", "不限速极速下载", "支持无损原画", "24/7 优先支持", "专属钻石徽章"], isPaid: true }] as tier}
              <!-- svelte-ignore a11y_click_events_have_key_events -->
              <!-- svelte-ignore a11y_no_static_element_interactions -->
              <div
                class="tier-card {memberLevel().label === tier.label
                  ? 'tier-active'
                  : ''} {tier.isPaid ? 'tier-clickable' : ''}"
                style="--tcolor: {tier.color}; --tbg: {tier.bg}"
                onclick={() => {
                  if (tier.isPaid) showPaymentModal = true;
                }}
              >
          ...
          <div class="progress-card">
            <div class="progress-header">
              ...
              <span class="progress-days" style="color: {memberLevel().color}"
                >{memberLevel().days} 天</span
              >
            </div>
            ...
            <div class="progress-bar-track">
              <div
                ...
                style="width: {progressPct}%; background: {memberLevel().color}"
              ></div>
            </div>
          ...
          <div class="stats-row">
            ...
            <div class="stat-box">
              <div class="stat-val" style="color:{memberLevel().color}">
                {memberLevel().icon}
                {memberLevel().label}
              </div>
            ...
            <div class="readonly-section">
              ...
              <div class="readonly-fields">
                ...
                <div class="readonly-field">
                  ...
                  <span class="readonly-val mono"
                    >{user?.id?.slice(0, 16)}…</span
                  >
                </div>
              </div>
            </div>
    ...
    >
      <div class="modal-content" onclick={(e) => e.stopPropagation()}>
        ...
        <div class="modal-header">
          ...
          <h3 class="modal-title">升级至白金会员</h3>
          <p class="modal-sub">解锁所有顶级权益，享受极致体验</p>
        </div>

        <div class="plan-cards">
          <!-- Monthly Plan -->
          <a
            href="https://www.creem.io/test/payment/prod_3NqAIWLXzxSQIbJ8vZusio{user?.id
            ...
            class="plan-card"
          >
            <div class="plan-name">月付方案</div>
            <div class="plan-price">
              $10 <span class="plan-period">/ 月</span>
            </div>
            <ul class="plan-features">
              <li>随用随付，更加灵活</li>
              <li>享受所有白金特权</li>
            </ul>
            <button class="btn-primary plan-btn">立即订阅</button>
          </a>

          <!-- Yearly Plan -->
          <a
            href="https://www.creem.io/test/payment/prod_Vd7S958QVPfbSxsZsNdiR{user?.id
              ? '?metadata[user_id]=' + user.id
              : ''}"
            target="_blank"
            rel="noopener noreferrer"
          ...
          >
            <div class="plan-badge">立省 40%</div>
            <div class="plan-name">年付方案</div>
            <div class="plan-price">
              $69.99 <span class="plan-period">/ 年</span>
            </div>
            <ul class="plan-features">
              <li>年度订阅超值优惠</li>
              <li>尊享专属钻石徽章</li>
            </ul>
            <button class="btn-primary plan-btn btn-featured">超值订阅</button>
          </a>
        </div>
      </div>
    </div>
...
<style>
  ...
  .btn-danger:hover {
    box-shadow: 0 6px 25px rgba(219, 39, 119, 0.4);
  }
  ...
  .tier-clickable:hover::after {
    box-shadow: inset 0 0 0 2px var(--tcolor);
    background: rgba(255, 255, 255, 0.05);
  }
